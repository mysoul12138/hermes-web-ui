...
function summarizeKnownMultimodalContent(value: unknown): string | null {
  if (Array.isArray(value)) return summarizeContentParts(value)
  if (isPlainRecord(value)) return summarizeMultimodalEnvelope(value)
  return null
}
  ...
  if (typeof content === 'string') {
    if (shouldTryParseStructuredString(content)) {
      try {
        ...
        // Fall back to regex redaction for malformed structured strings.
      }
    }
