...
export class HermesSkillInjector {
  ...
  static resolveHermesRootHome(env: NodeJS.ProcessEnv = process.env): string {
    return resolve(env.HERMES_HOME?.trim() || join(homedir(), '.hermes'))
  }
  ...
  static resolveTargetDirs(rootDir = HermesSkillInjector.resolveHermesRootHome()): string[] {
  ...
  static resolveTargetDirForProfile(profile: string, rootDir = HermesSkillInjector.resolveHermesRootHome()): string {
  ...
  private static profileForTargetDir(targetDir: string, rootDir = HermesSkillInjector.resolveHermesRootHome()): string {
    const root = resolve(rootDir)
