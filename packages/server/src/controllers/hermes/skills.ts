...
export async function listFiles(ctx: any) {
  ...
  const skillsDir = join(hd, 'skills')
  try {
    const config = await readConfigYaml()
    const skillDir = await resolveSkillDirFromConfig(config, skillsDir, category, skill)
    if (!skillDir) {
      ctx.status = 404
...
export async function readFile_(ctx: any) {
  ...
  const fullPath = resolve(join(skillsDir, realPath))
  if (!isPathWithin(fullPath, skillsDir)) {
    ctx.status = 403
  ...
  if (content === null) {
    ...
    if (parts.length >= 2) {
      ...
      const config = await readConfigYaml()
      const skillDir = await resolveSkillDirFromConfig(config, skillsDir, category, skillName)
      if (skillDir) {
