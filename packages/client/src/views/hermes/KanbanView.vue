...
const profileAvatarByName = computed<Record<string, KanbanProfileAvatarData | null>>(() => {
  const profiles = profilesStore.profiles as Array<{ name: string; avatar?: KanbanProfileAvatarData | null }>
  return Object.fromEntries(profiles.map(profile => [profile.name, profile.avatar || null]))
})
